...
internal class XLinearCalendarViewModel : ViewModel() {

    val uiState: MutableStateFlow<XLinearCalendarUiModel> = MutableStateFlow(XLinearCalendarUiModel())
     ...
     */
    /**
     * Initialize the calendar with a custom list of dates if provided.
     */
    fun loadInitialDates(initialDates: List<LocalDate>? = null) {
        val today = LocalDate.now()

        val dates = initialDates ?: ((-DAYS_TO_LOAD)..DAYS_TO_LOAD).map { today.plus(DatePeriod(days = it)) }
        val selected = if (initialDates != null) initialDates.firstOrNull() ?: today else today
    ...
    fun loadPreviousDates() {
        Log.e("LSS","LOAD PREVIOUS CALLED")
    ...
    fun setInitialDates(initialDates: List<Date>) {
